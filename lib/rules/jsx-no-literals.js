  ...
  create(context) {
    ...
    function reportLiteralNode(node, customMessage) {
      ...
      context.report({
        ...
        fix(fixer) {
          const wrapLineRegex = /^\s*(\S.*)$/gm; // any whitespace followed by some non-whitespace
          const sourceCodeText = context.getSourceCode().getText(node);
          const replaceSourceCode = sourceCodeText.replace(wrapLineRegex,
            (match, m1) => m1.replace(/^(\s*)(.+)$/gm, '$1{t(\'$2\')}'));
          return [
            fixer.replaceText(node, replaceSourceCode)
          ];
        }
      });
    }
